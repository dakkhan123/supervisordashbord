...
const Sidebar = ({ mobileOpen, setMobileOpen, alertCount = 0 }) => {
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 w-[260px] h-screen bg-[#213145] flex flex-col py-5 z-100 transition-transform duration-250 ease-in-out overflow-y-auto overflow-x-hidden z-[100] lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-3 border-t border-white/8 mt-auto">
          <div className="flex items-center gap-2.5 p-2.5 rounded-lg hover:bg-white/8 cursor-pointer transition-colors duration-150">
            <img 
              ...
              alt="Supervisor Rajesh Kumar"
            />
            <div className="flex-1">
              <p className="text-xs font-semibold text-primary-fixed">Rajesh Kumar</p>
              <span className="text-[10px] text-secondary-fixed-dim/70 opacity-70">Unit Pune-A12</span>
            </div>
            <span className="material-symbols-outlined icon-xs text-secondary-fixed-dim/50">more_vert</span>
          </div>
        </div>
      </aside>
    </>
