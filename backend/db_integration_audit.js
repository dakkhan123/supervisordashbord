...
async function runAudit() {
  ...
  try {
    ...
    if (token) {
      ...
      const protectedRes = await fetch(`${SERVER_URL}/api/workers`, {
    ...
    try {
      const unauthorizedRes = await fetch(`${SERVER_URL}/api/workers`);
      if (unauthorizedRes.status === 401) {
        console.log('✅ Protected route correctly rejected unauthorized request (401 Unauthorized).');
      } else {
        console.warn('❌ Protected route did not reject request with 401. Status:', unauthorizedRes.status);
        report.authWorking = 'NO';
      }
    } catch (err) {
      console.warn('❌ Protected route connection error:', err.message);
      report.authWorking = 'NO';
    }
    ...
    try {
      const badReqRes = await fetch(`${SERVER_URL}/api/auth/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (badReqRes.status === 400) {
        ...
        console.warn('❌ Registration endpoint did not reject request with 400. Status:', badReqRes.status);
      }
    } catch (err) {
      console.warn('❌ Connection error on validation test:', err.message);
    }
